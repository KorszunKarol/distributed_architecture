...
class HeavyweightProcessA(HeavyweightProcess):
    ...
    def __init__(self, number: int = 0, port: int = 0):
        """Initialize heavyweight process A.
    ...
    async def pass_token(self) -> None:
        """Pass token to heavyweight process B."""
        ...
        await self.send_message(token_msg, NetworkConfig.HEAVYWEIGHT_B_PORT)
        self.logger.info("Passed token to HWB")
