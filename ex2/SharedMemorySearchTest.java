...
public class SharedMemorySearchTest {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Starting Performance Tests (Shared Memory)");
        System.out.println("=========================================");
        ...
        for (int size : arraySizes) {
            for (int threads : threadCounts) {
                ...
                for (int iter = 0; iter < iterations; iter++) {
                    ...
                    int searchResult = SharedMemoryParallelSearch.parallelSearch(size/2, array, threads);
                    ...
                    // Add small delay between iterations to reduce cache effects
                    try {
                        Thread.sleep(10);
        ...
        // Save results to CSV
        try {
            saveResultsToCSV(allResults, "shared_memory_search_results.csv");
            System.out.println("\nResults saved to shared_memory_search_results.csv");
        } catch (IOException e) {
    ...
    private static void saveResultsToCSV(List<TestResult> results, String filename) throws IOException {
        try (FileWriter writer = new FileWriter(filename)) {
            // Write header with 20 iterations
            writer.write("ArraySize,ThreadCount,AverageTime");
            for (int i = 1; i <= 20; i++) {
                writer.write(",Time" + i);
            }
            writer.write("\n");
