"""Third layer node implementing lazy, passive replication with primary-backup."""
...
class ThirdLayerNode(BaseNode):
    """Third layer node that receives updates every 10 seconds through passive replication.

    This node belongs to layer 2 and implements:
    ...
    - Lazy replication: Syncs every 10 seconds
    """
    ...
    async def start(self):
        ...
        print(f"Starting third layer node {self.node_id}")
        await super().start()
        print(f"Third layer node {self.node_id} server started")
        ...
        print(f"Third layer node {self.node_id} tasks created")
        return self
    ...
    async def _sync_loop(self):
        """Sync with appropriate source every 10 seconds."""
        while not self._closed:
            try:
                current_time = time.time()
                if current_time - self.last_sync_time >= 10:
                    if self.is_primary:
    ...
    async def ExecuteTransaction(
        ...
        try:
            if request.type != replication_pb2.Transaction.READ_ONLY:
                raise Exception("Third layer nodes only accept read-only transactions")
